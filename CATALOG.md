# Каталог дополнительных коллекций скиллов

Основное ядро из 21 скилла уже лежит в `skills/` и устанавливается целиком. Ниже перечислены необязательные внешние коллекции для задач, которых нет в ядре.

Все репозитории принадлежат своим авторам. Перед копированием конкретного скилла проверь его актуальную лицензию. Ссылка в этом каталоге не означает разрешение на перепубликацию.
...
- Этот кит: готовое ядро из 21 скилла в папке `skills/`, полный состав в `README.md`.
